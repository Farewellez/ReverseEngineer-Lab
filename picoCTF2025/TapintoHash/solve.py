...
def decrypt(cipher, key):
    # midpoint = len(cipher)//2

    # first_part = cipher[:midpoint]
    # second_part = cipher[midpoint:]
    blocksize = 16
    key_hash = hashlib.sha256(key).digest()

    raw_cipher = b''
    for i in range(0,len(cipher),blocksize):
        cipher_block  = cipher[i:i+blocksize]
        xored_cipher = xor_bytes(cipher_block,key_hash)
        raw_cipher += xored_cipher
    print("Raw cipher:",raw_cipher,len(raw_cipher))
    return raw_cipher

def xor_bytes(a, b):
    return bytes(x ^ y for x, y in zip(a, b))

enc_flag = open("./enc_flag",'r').read()
...
hex_enc_bc = Encrypted_Blockchain.hex()
hex_key = key.hex()
print("hex encrypted blockchain:",hex_enc_bc,len(Encrypted_Blockchain))
print("hex key:",hex_key)

candidate = decrypt(Encrypted_Blockchain,key)
print(candidate[192-(16*2):])